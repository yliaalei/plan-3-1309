...
function initApp(){
  ...
  function renderCalendar(){
    ...
    for(let d=1; d<=daysInMonth; d++){
      ...
      // load saved doc to set color and icons
      dbRef.doc(key).get().then(docSnap => {
        ...
        // icons: create for each type if platform flags exist
        iconsWrap.innerHTML = "";
        ["post","reel","stories"].forEach(type => {
          const flags = data[`${type}Platforms`] || {};
          // if any platform true, show a small combined icon (optional)
          if(flags.vk || flags.inst || flags.tg) {
            const img = document.createElement("img");
            img.src = ICONS.inst; // small indicator (using instagram icon as marker)
            img.alt = type;
            img.title = type;
  ...
  function saveEditor(){
    ...
    dbRef.doc(selectedDateKey).set(payload, { merge: true }).then(()=> {
      // update calendar icons for that date
      renderCalendar();
    }).catch(e => console.error(e));
  }
  let saveTimer;
