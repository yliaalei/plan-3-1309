...
function initApp() {
  ...
  function updateCalendarBackground(month) {
    ...
    if ([8,9,10].includes(month)) { // Сентябрь, Октябрь, Ноябрь
      calendarSection.style.backgroundImage = "linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('https://i.pinimg.com/736x/90/1c/6a/901c6aab908ff55adc594fabae3ace52.jpg')";
  ...
  function copyEditorText(){
    ...
    navigator.clipboard.writeText(quill.root.innerHTML).then(()=>{
      ...
      btn.textContent="Скопировано!"; setTimeout(()=>btn.textContent=old,1000);
    });
  }
