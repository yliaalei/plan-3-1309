// Используем уже объявленные в firebase-config.js переменные: db и auth
...
function initApp(){
  ...
  safeAssign("prevBtn","onclick", () => {
    currentMonth--; if(currentMonth<0){ currentMonth=11; currentYear--; }
    renderCalendar();
  });
  safeAssign("nextBtn","onclick", () => {
  ...
  function copyEditorText(){
    ...
    navigator.clipboard.writeText(quill.root.innerHTML).then(()=>{
      const btn=$("copyBtn"); const old=btn.textContent; btn.textContent="Скопировано!";
