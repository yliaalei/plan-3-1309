(() => {
  ...
  // === Инициализация календаря и редакторов ===
  function initApp(){
      ...
      for(let d=1; d<=daysInMonth; d++){
        ...
        dbRef.doc(key).get().then(doc => {
          if(doc.exists){
            ...
            cell.style.backgroundColor = base === "#ffffff" ? "rgba(255,255,255,0.15)" : hexToRgba(base,0.3);
          }
        });
    ...
    function updateCalendarBackground(month){
      ...
      if([8,9,10].includes(month)){
        bg.style.backgroundImage = "url('https://i.pinimg.com/736x/90/1c/6a/901c6aab908ff55adc594fabae3ace52.jpg')";
    ...
    function showEditor(type){
      ...
      if(checks.childElementCount===0){
        checks.innerHTML = `<label><input type="checkbox" id="chk_vk"> VK</label>
                            <label><input type="checkbox" id="chk_inst"> Instagram</label>
                            <label><input type="checkbox" id="chk_tg"> Telegram</label>`;
    ...
    function saveEditor(){
      ...
      dbRef.doc(selectedDateKey).set({ [selectedType]:val, [`${selectedType}Platforms`]:flags },{merge:true}).then(()=>renderCalendarIcons(selectedDateKey));
    }
